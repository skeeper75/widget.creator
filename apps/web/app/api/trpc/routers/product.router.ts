...
export const productRouter = router({
  ...
  clone: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      // Fetch source product (outside transaction for early validation)
      ...
      return ctx.db.transaction(async (tx) => {
        ...
        for (const c of sourceConstraints) {
          await tx.insert(optionConstraints).values({
            productId: newProductId,
        ...
        for (const d of sourceDeps) {
          await tx.insert(optionDependencies).values({
            productId: newProductId,
        ...
        // Clone fixed prices
        const sourceFixedPrices = await tx
          .select()
          .from(fixedPrices)
          .where(eq(fixedPrices.productId, id));

        for (const fp of sourceFixedPrices) {
          await tx.insert(fixedPrices).values({
            productId: newProductId,
        ...
        const sourcePackagePrices = await tx
          .select()
          .from(packagePrices)
